...
export default function StudentFormPage() {
  ...
  const handleNrpSearch = async (targetNrp: string) => {
    ...
    try {
      ...
      if (entryData) {
        setProdi(entryData.prodi || autoProdi);
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 shadow-sm border border-slate-200/80 dark:border-slate-800 flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <span className="text-[11px] text-slate-400">
                  (Otomatis ter-default berdasarkan awalan NRP, namun tetap dapat diubah jika berbeda)
                </span>
              </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 shadow-sm border border-slate-200/80 dark:border-slate-800 flex flex-col gap-4">
              ...
              {fotoPreview ? (
                <div className="flex flex-col gap-4">
                  {/* Image Preview */}
                  <div className="relative w-full aspect-[4/3] rounded-2xl overflow-hidden shadow-md bg-slate-900 border border-slate-200 dark:border-slate-800 group">
                    ...
                    {geotagTimestamp && (
                      <div className="absolute bottom-3 left-3 right-3 text-white text-xs space-y-1">
                        <div className="flex items-center gap-1 font-semibold text-emerald-400">
                          ...
                          <span>Foto & Geotag Terekam</span>
                        </div>
                        <p className="text-[11px] text-slate-200 opacity-90 truncate">{geotagAddress}</p>
                        <p className="text-[10px] text-slate-400">
                          Waktu: {new Date(geotagTimestamp).toLocaleString('id-ID')}
                        </p>
                      </div>
                    )}
                  </div>
